...
plot ( rnorm (200), pch = 18, size = 0.3)                   #random numbers
...
apply ( USArrests , 2, mean ) # row & vector 마다 계산을 해준다. 1이면 row별로 2이면 column별로
...
#library(ISwR)
lapply( thuesen, mean, na.rm=T) # list형태로 출력
sapply ( thuesen , mean , na.rm=T) # 가능한한 간단하게, vector이 자주 되는듯
...
hist.with.normal <- function (x, xlab = deparse (substitute(x)) ,...){ # deparse = as.character
  h <- hist (x, plot =F, ...)
  s <- sd(x)
  m <- mean (x)
